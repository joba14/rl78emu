	...
	const uint64_t size)
{
	...
	if (NULL == pointer)
	{
		rl78emu_misc_logger_error("internal failure -- failed to allocate memory.");
		rl78emu_misc_exit(-1);
	}
	...
	const uint64_t size)
{
	...
	if (NULL == pointer)
	{
		rl78emu_misc_logger_error("internal failure -- failed to reallocate memory.");
		rl78emu_misc_exit(-1);
	}
...
{
	rl78emu_misc_debug_assert(pointer != NULL);
...
void rl78emu_misc_memcpy(
	...
	const uint64_t length)
{
	rl78emu_misc_debug_assert(destination != NULL);
	rl78emu_misc_debug_assert(source != NULL);
	rl78emu_misc_debug_assert(length > 0);
...
{
	rl78emu_misc_debug_assert(cstring != NULL);
...
{
	rl78emu_misc_debug_assert(left != NULL);
	rl78emu_misc_debug_assert(right != NULL);
