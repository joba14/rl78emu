...
{
	rl78emu_misc_debug_assert(argc > 0);
	rl78emu_misc_debug_assert(argv != NULL);
	...
	for (uint64_t argv_index = 1; argv_index < argc; ++argv_index)
	{
		...
		rl78emu_misc_debug_assert(option != NULL);

		if (match_option(option, "--help", "-h"))
		{
			rl78emu_cli_config_usage();
			rl78emu_misc_exit(0);
		}
		else if (match_option(option, "--version", "-v"))
		{
			// todo: handle this:
			// rl78emu_misc_logger_log("%s %s", g_program, rl78emu_cli_version);
			rl78emu_misc_exit(0);
		}
		else
		{
			if (binary != NULL)
			{
				rl78emu_misc_logger_error("binary path was already provided: '%s'. invalid command line option '%s' was provided.", binary, option);
				rl78emu_cli_config_usage();
				rl78emu_misc_exit(-1);
			}
	...
	if (NULL == binary)
	{
		rl78emu_misc_logger_error("missing required binary path argument.");
		rl78emu_cli_config_usage();
		rl78emu_misc_exit(-1);
	}
...
void rl78emu_cli_config_usage(
	void)
{
	rl78emu_misc_debug_assert(g_usage_banner != NULL);
	rl78emu_misc_debug_assert(g_program != NULL);
	rl78emu_misc_logger_log(g_usage_banner, g_program);
}
...
{
	rl78emu_misc_debug_assert(option != NULL);
	rl78emu_misc_debug_assert(long_name != NULL);
	rl78emu_misc_debug_assert(short_name != NULL);

	const uint64_t option_length = rl78emu_misc_strlen(option);
	const uint64_t long_name_length = rl78emu_misc_strlen(long_name);
	const uint64_t short_name_length = rl78emu_misc_strlen(short_name);

	return (
		((option_length == long_name_length) && rl78emu_misc_strncmp(option, long_name, option_length) == 0) ||
		((option_length == short_name_length) && rl78emu_misc_strncmp(option, short_name, option_length) == 0)
	);
}
